...
// 定义可以通过 next() 传入迭代器的值的类型
type IteratorCommand = 'hurry' | 'pause' | undefined; // 'hurry': 加快; 'pause': 暂停(演示用); undefined: 无命令

// 我们的主类，实现了 AsyncIterable
// T = string (倒计时消息的类型)
// TReturn = CountdownResult (最终完成时返回值的类型)
// TNext = IteratorCommand (可以发送给迭代器的命令的类型)
class AsyncCountdown implements AsyncIterable<string, CountdownResult, IteratorCommand> {
  private startCount: number; // 起始计数
  private intervalMs: number; // 每次计数的间隔时间 (毫秒)
  ...
  [Symbol.asyncIterator](): AsyncIterator<string, CountdownResult, IteratorCommand> {
    ...
    return {
      ...
      async next(command?: IteratorCommand): Promise<IteratorResult<string, CountdownResult>> {
      ...
      async return(
        value?: CountdownResult | PromiseLike<CountdownResult>,
      ): Promise<IteratorResult<string, CountdownResult>> {
      ...
      async throw(e?: any): Promise<IteratorResult<string, CountdownResult>> {
...
async function runCountdownNormally() {
  console.log('\n--- 示例1: 使用 for await...of 正常运行倒计时 ---');
  const countdown = new AsyncCountdown(3, 700); // 从 3 开始, 700ms 间隔

  try {
    // for await...of 循环自动处理调用 .next()
    // 并解开 'value' 直到 'done' 为 true。
    // 重要: 这个循环本身 *不会* 让你直接访问到迭代完成时的 TReturn 值 (最终的那个 CountdownResult)。
    ...
    // 注意: 我们在这里无法自动获取到最终的 CountdownResult。
  } catch (error) {
    console.error('for await...of 循环出错:', error);
  }
}

async function runCountdownWithReturn() {
  console.log('\n--- 示例2: 使用 return() 提前停止迭代并传递值 ---');
  const countdown = new AsyncCountdown(5, 1000); // 从 5 开始, 1 秒间隔

  // 获取迭代器实例
  const iterator = countdown[Symbol.asyncIterator]();

  let result: IteratorResult<string, CountdownResult> | undefined;
  ...
  try {
    ...
    while (!result.done) {
      ...
      if (result.value.includes('3')) {
        ...
        // 调用 return() 方法，传递一个 CountdownResult 对象
        if (iterator.return) {
...
async function runCountdownManually() {
  ...
  let result: IteratorResult<string, CountdownResult> | undefined; // 用来存储每次 next() 的结果
  ...
  try {
    ...
    while (!result.done) {
      ...
      let command: IteratorCommand = undefined;
    ...
    console.log('手动迭代完成。');
    console.log('最终结果 (TReturn):', result.value); // result.value 的类型是 CountdownResult
...
// --- 运行主函数 ---
async function main() {
  await runCountdownNormally();
  await runCountdownWithReturn();
  await runCountdownManually();
}

main().catch(console.error); // 捕获顶层可能出现的任何错误
