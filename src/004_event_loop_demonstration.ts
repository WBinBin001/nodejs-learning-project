...
class EventLoopDemonstration {
   ...
   * 打印预期的输出顺序（仅供参考）
   */
  public printExpectedOutput(): void {
    console.log(`
    --- 预测输出顺序 ---
    1.  '--- 脚本开始 ---'                   (同步)
...
// 对外暴露的测试函数
export function event_loop_demonstration(): void {
  const demo = new EventLoopDemonstration();
  demo.test();
}
