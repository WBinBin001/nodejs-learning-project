...
async function practiceBasicPromise() {
  console.log('\n--- 1. Promise 基本使用 ---');

  // 示例 1: 成功的 Promise
  const successTask = createAsyncTask('读取文件', 500, true);
  successTask
    .then((result: string) => {
  ...
  await delay(600); // 等待上一个任务完成

  // 示例 2: 失败的 Promise
  const failureTask = createAsyncTask('写入数据库', 400, false);
  failureTask
    .then((result: string) => {
      // 这个 then 不会执行
  ...
  await delay(500); // 等待上一个任务完成
}

// --- 2. .finally() ---
async function practiceFinally() {
  console.log('\n--- 2. .finally() ---');
  // finally 无论 Promise 是成功还是失败，都会执行，常用于清理工作

  const task1 = createAsyncTask('资源操作 (成功)', 300, true);
  task1
  ...
  const task2 = createAsyncTask('资源操作 (失败)', 200, false);
  task2
    .then((result) => console.log(`  ➡️ then: ${result}`))
    .catch((error) => console.error(`  ❌ catch: ${error.message}`))
    .finally(() => {
...
async function practiceAsyncAwait() {
  ...
  async function processTasks(): Promise<string> {
    ...
    try {
      ...
      const resultA = await createAsyncTask('TaskA', 600, true); // 等待 Promise 解决
      console.log(`  TaskA 完成，结果: "${resultA}"`);

      console.log('  等待 taskB (将失败)...');
      const resultB = await createAsyncTask('TaskB', 300, false); // 等待 Promise 拒绝
  ...
  await delay(1200); // 等待 processTasks 完成
}

// --- 4. Promise 静态方法 ---
async function practiceStaticMethods() {
  console.log('\n--- 4. Promise 静态方法 ---');

  const promise1 = createAsyncTask('静态 P1', 700, true);
  const promise2 = createAsyncTask('静态 P2', 300, true);
  const promise3 = createAsyncTask('静态 P3 (失败)', 500, false).catch(() => {
    /* 添加空的 catch 来防止未处理的拒绝错误 */
  });
  const promise4 = delay(400).then(() => '[静态 P4] (来自 delay)'); // 另一个成功的 Promise
  ...
  try {
    ...
    const resultsAllFailure = await Promise.all([
      createAsyncTask('重试 P1', 100, true),
      createAsyncTask('重试 P2', 50, true),
      createAsyncTask('重试 P3 (失败)', 200, false), // 这个会失败
    ]);
  ...
  try {
    ...
    const resultRaceSuccess = await Promise.race([
      createAsyncTask('赛跑 P1 (慢)', 700, true),
      createAsyncTask('赛跑 P2 (快)', 300, true),
    ]);
  ...
  try {
    ...
    const resultRaceSuccessFirst = await Promise.race([
      createAsyncTask('赛跑 P3 (慢失败)', 500, false),
      delay(400).then(() => '[赛跑 P4] (快成功)'),
    ]);
  ...
  try {
    ...
    const resultRaceFailureFirst = await Promise.race([
      createAsyncTask('赛跑 P1 (慢成功)', 700, true),
      createAsyncTask('赛跑 P3 (快失败)', 500, false),
    ]);
  ...
  const allSettledPromises = [
    createAsyncTask('安定 P1', 150, true),
    createAsyncTask('安定 P2 (失败)', 300, false),
    delay(100).then(() => '[安定 P3]'),
  ];
  ...
  try {
    ...
    const anySuccessResult = await Promise.any([
      createAsyncTask('任一 P3 (失败)', 500, false),
      createAsyncTask('任一 P2 (快成功)', 200, true),
      createAsyncTask('任一 P1 (慢成功)', 700, true),
    ]);
  ...
  try {
    ...
    const anyFailureResult = await Promise.any([
      createAsyncTask('任一 P3 (失败)', 100, false),
      createAsyncTask('任一 P5 (失败)', 200, false),
    ]);
...
// --- 主执行函数 ---
async function main() {
  await practiceBasicPromise();
  await practiceFinally();
  await practiceAsyncAwait();
  await practiceStaticMethods();
  console.log('\n--- Promise 练习结束 ---');
}
